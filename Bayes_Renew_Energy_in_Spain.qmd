...
```{r}
barcelona_1 <- subset(barcelona_weather_data, select = -c(city_name, weather_id, `generation hydro pumped storage aggregated`, `forecast wind offshore eday ahead`, rain_1h, rain_3h, snow_3h))
...
```{r}
ggplot(data = barcelona_1, aes(y = `generation waste`, fill = year)) +
  geom_boxplot()
```
