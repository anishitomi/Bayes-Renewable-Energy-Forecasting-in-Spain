...
```{r}
prediction <- 
  posterior_predict(barcelona_non_renewable_energy_sources, newdata = new_data_for_prediction_nonrenewable)
```
...
```{r}
new_data <- data.frame(`generation fossil brown coal/lignite` = 300,
...
```{r}
demand_forecast <- posterior_predict(demand_model, newdata = new_data)

renewable_forecast <- posterior_predict(renewable_model, newdata = new_data)
